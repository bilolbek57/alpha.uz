...
function CartItem({karzinka}) {
  ...
  return (
    <div>
        ...
        <div className="">
            {
                karzinka?.map((item, inx)=> <div key={inx} className="">
                    <img src={item?.urls[0]} width={100} alt="" />
                     <Link to={`/product/${item.id}`}><p>{item?.title}</p></Link>
                  <p>{item?.price.brm()} so'm</p> 
                    <button disabled={ item?.qty <= 1 } onClick={() => removeCart(item)}>-</button>
                    <span>{item?.qty}</span>
                    <button onClick={() => addToCart(item)}>+</button>
                    <button onClick={()=> dispatch({type: REMOVE_CART, payload: item.id})} >delete</button>
                </div>)
            } 
        </div>
    </div>
